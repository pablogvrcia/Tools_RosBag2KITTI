										...
										init_camera_time(false), init_lidar_time(false)
{
	...
	//sub_lidar = nh.subscribe("/ibeo_sdk/scan_147", 1000, &MapGenerationNode::lidarCallback, this);
	sub_lidar = nh.subscribe("/ibeo_sdk/scan_20", 1000, &MapGenerationNode::lidarCallback, this);
	...
	//sub_camera = it.subscribe("/ibeo_sdk/camera5", 1000, &MapGenerationNode::cameraCallback, this);*/
	sub_camera = it.subscribe("/ibeo_sdk/camera_3", 1000, &MapGenerationNode::cameraCallback, this);
...
{
	std::cout << "New Lidar Message: " << lidar << std::endl;
	if(!init_lidar_time)
	{
...
{
	std::cout << "New Camera Message: " << camera << std::endl;
	if(!init_camera_time)
	{
